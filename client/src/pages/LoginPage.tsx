...
const DEV_STAFF_LOGIN_STORAGE_KEY = "dev_staff_login_accounts";
...
export function LoginPage() {
  ...
  const loadSavedAccounts = () => {
    ...
    setSavedAccounts(loadDevLoginAccounts(DEV_STAFF_LOGIN_STORAGE_KEY));
  ...
  const removeSavedAccount = (accountEmail: string) => {
    setSavedAccounts(
      removeDevLoginAccount(DEV_STAFF_LOGIN_STORAGE_KEY, accountEmail),
    );
  };
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (import.meta.env.DEV) {
        saveDevLoginAccount(DEV_STAFF_LOGIN_STORAGE_KEY, { email, password });
      }
      ...
      <section className="mx-auto grid max-w-6xl gap-10 px-2 py-12 lg:grid-cols-[1fr_440px] lg:items-center">
        <div>
          ...
          <p className="mt-5 max-w-2xl text-base leading-7 text-slate-600">
            Staff can manage tickets, requesters, agents, knowledge, and
            workflows from one tenant-aware SaaS console.
          </p>
        </div>
          ...
          <div className="relative rounded-[1.5rem] border border-slate-200 bg-white p-6 shadow-2xl shadow-slate-200 sm:p-7">
            <div className="mb-6">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-500">
                Sign in to manage tickets, requesters, and support workflows.
              </p>
            </div>
