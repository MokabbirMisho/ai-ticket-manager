import { type ReactNode, useEffect, useState } from "react";
import { Navigate } from "react-router-dom";
import { api } from "../api/axios";
import { useAuth } from "../context/AuthContext";

type Requester = {
  id: string;
  name: string;
  email: string;
  phone: string | null;
  company: string | null;
  department: string | null;
  jobTitle: string | null;
  country: string | null;
  city: string | null;
  address: string | null;
  isActive: boolean;
  createdAt: string;
  updatedAt: string;
  _count?: {
    tickets: number;
  };
};

type Pagination = {
  page: number;
  limit: number;
  totalRequesters?: number;
  totalStudents?: number;
  totalPages: number;
};

type RequesterFormState = {
  name: string;
  email: string;
  password: string;
  phone: string;
  company: string;
  department: string;
  jobTitle: string;
  country: string;
  city: string;
  address: string;
  isActive: boolean;
};

const emptyForm: RequesterFormState = {
  name: "",
  email: "",
  password: "",
  phone: "",
  company: "",
  department: "",
  jobTitle: "",
  country: "",
  city: "",
  address: "",
  isActive: true,
};

const isValidEmail = (value: string) => {
  return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value);
};

const getRequiredProfileError = (
  form: RequesterFormState,
  options: { requirePassword: boolean },
) => {
  if (!form.name.trim()) return "Name is required";
  if (!form.email.trim()) return "Email is required";
  if (!isValidEmail(form.email.trim())) return "A valid email is required";
  if (options.requirePassword && !form.password.trim()) {
    return "Temporary password is required";
  }
  if (!form.phone.trim()) return "Phone is required";
  if (!form.company.trim()) return "Company / Organization is required";
  if (!form.country.trim()) return "Country is required";
  if (!form.city.trim()) return "City is required";
  if (!form.address.trim()) return "Address is required";

  return "";
};

export function RequestersPage() {
  const { user } = useAuth();

  const [requesters, setRequesters] = useState<Requester[]>([]);
  const [pagination, setPagination] = useState<Pagination>({
    page: 1,
    limit: 20,
    totalRequesters: 0,
    totalPages: 1,
  });

  const [createForm, setCreateForm] = useState<RequesterFormState>(emptyForm);
  const [editForm, setEditForm] = useState<RequesterFormState>(emptyForm);

  const [search, setSearch] = useState("");
  const [status, setStatus] = useState("");

  const [selectedRequester, setSelectedRequester] = useState<Requester | null>(
    null,
  );

  const [isLoading, setIsLoading] = useState(true);
  const [isCreating, setIsCreating] = useState(false);
  const [isUpdating, setIsUpdating] = useState(false);
  const [error, setError] = useState("");
  const [success, setSuccess] = useState("");

  if (user?.role !== "ADMIN") {
    return <Navigate to="/dashboard" replace />;
  }

  const requesterTotal =
    pagination.totalRequesters ?? pagination.totalStudents ?? 0;

  const updateCreateForm = (
    field: keyof RequesterFormState,
    value: string | boolean,
  ) => {
    setCreateForm((current) => ({ ...current, [field]: value }));
  };

  const updateEditForm = (
    field: keyof RequesterFormState,
    value: string | boolean,
  ) => {
    setEditForm((current) => ({ ...current, [field]: value }));
  };

  const fetchRequesters = async (page = pagination.page) => {
    try {
      setIsLoading(true);
      setError("");

      const response = await api.get("/requesters", {
        params: {
          page,
          limit: pagination.limit,
          search: search || undefined,
          status: status || undefined,
        },
      });

      setRequesters(response.data.data.requesters ?? response.data.data.students ?? []);
      setPagination(response.data.data.pagination);
    } catch {
      setError("Failed to load requesters");
    } finally {
      setIsLoading(false);
    }
  };

  useEffect(() => {
    fetchRequesters(1);
  }, [search, status]);

  const handleCreateRequester = async (
    event: React.FormEvent<HTMLFormElement>,
  ) => {
    event.preventDefault();

    const validationError = getRequiredProfileError(createForm, {
      requirePassword: true,
    });

    if (validationError) {
      setError(validationError);
      return;
    }

    try {
      setIsCreating(true);
      setError("");
      setSuccess("");

      await api.post("/requesters", {
        ...createForm,
        email: createForm.email.trim(),
      });

      setSuccess("Requester created successfully");
      setCreateForm(emptyForm);
      await fetchRequesters(1);
    } catch {
      setError("Failed to create requester");
    } finally {
      setIsCreating(false);
    }
  };

  const openEditModal = (requester: Requester) => {
    setSelectedRequester(requester);
    setEditForm({
      name: requester.name,
      email: requester.email,
      password: "",
      phone: requester.phone ?? "",
      company: requester.company ?? "",
      department: requester.department ?? "",
      jobTitle: requester.jobTitle ?? "",
      country: requester.country ?? "",
      city: requester.city ?? "",
      address: requester.address ?? "",
      isActive: requester.isActive,
    });
    setError("");
    setSuccess("");
  };

  const closeEditModal = () => {
    setSelectedRequester(null);
    setEditForm(emptyForm);
  };

  const handleUpdateRequester = async () => {
    if (!selectedRequester) return;

    const validationError = getRequiredProfileError(editForm, {
      requirePassword: false,
    });

    if (validationError) {
      setError(validationError);
      return;
    }

    try {
      setIsUpdating(true);
      setError("");
      setSuccess("");

      const payload = {
        ...editForm,
        email: editForm.email.trim(),
        password: editForm.password.trim() || undefined,
      };

      await api.patch(`/requesters/${selectedRequester.id}`, payload);

      setSuccess("Requester updated successfully");
      closeEditModal();
      await fetchRequesters(pagination.page);
    } catch {
      setError("Failed to update requester");
    } finally {
      setIsUpdating(false);
    }
  };

  const handleDeactivateRequester = async (requesterId: string) => {
    try {
      setError("");
      setSuccess("");

      await api.delete(`/requesters/${requesterId}`);

      setSuccess("Requester deactivated successfully");
      await fetchRequesters(pagination.page);
    } catch {
      setError("Failed to deactivate requester");
    }
  };

  const goToPreviousPage = () => {
    if (pagination.page > 1) {
      fetchRequesters(pagination.page - 1);
    }
  };

  const goToNextPage = () => {
    if (pagination.page < pagination.totalPages) {
      fetchRequesters(pagination.page + 1);
    }
  };

  return (
    <div>
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Requesters</h1>
        <p className="mt-2 text-slate-500">
          Admin area to create and manage requester portal accounts.
        </p>
      </div>

      <section className="mt-6 rounded-2xl bg-white p-6 shadow-sm">
        <h2 className="text-lg font-bold text-slate-900">Create Requester</h2>

        <form onSubmit={handleCreateRequester} className="mt-5 space-y-6">
          <FormSection title="Basic Information">
            <TextField
              label="Name"
              value={createForm.name}
              onChange={(value) => updateCreateForm("name", value)}
              required
            />
            <TextField
              label="Email"
              type="email"
              value={createForm.email}
              onChange={(value) => updateCreateForm("email", value)}
              required
            />
            <TextField
              label="Temporary Password"
              type="password"
              value={createForm.password}
              onChange={(value) => updateCreateForm("password", value)}
              required
            />
          </FormSection>

          <FormSection title="Contact Information">
            <TextField
              label="Phone"
              value={createForm.phone}
              onChange={(value) => updateCreateForm("phone", value)}
              required
            />
            <TextField
              label="Country"
              value={createForm.country}
              onChange={(value) => updateCreateForm("country", value)}
              required
            />
            <TextField
              label="City"
              value={createForm.city}
              onChange={(value) => updateCreateForm("city", value)}
              required
            />
            <TextField
              label="Address"
              value={createForm.address}
              onChange={(value) => updateCreateForm("address", value)}
              required
              className="md:col-span-2"
            />
          </FormSection>

          <FormSection title="Organization Details">
            <TextField
              label="Company / Organization"
              value={createForm.company}
              onChange={(value) => updateCreateForm("company", value)}
              required
            />
            <TextField
              label="Department"
              value={createForm.department}
              onChange={(value) => updateCreateForm("department", value)}
            />
            <TextField
              label="Job Title"
              value={createForm.jobTitle}
              onChange={(value) => updateCreateForm("jobTitle", value)}
            />
          </FormSection>

          <FormSection title="Account Status">
            <StatusField
              value={createForm.isActive}
              onChange={(value) => updateCreateForm("isActive", value)}
            />
          </FormSection>

          <div className="flex justify-end">
            <button
              type="submit"
              disabled={isCreating}
              className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white disabled:opacity-60"
            >
              {isCreating ? "Creating..." : "Create Requester"}
            </button>
          </div>
        </form>

        {error && (
          <div className="mt-4 rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700">
            {error}
          </div>
        )}

        {success && (
          <div className="mt-4 rounded-lg bg-green-50 px-4 py-3 text-sm text-green-700">
            {success}
          </div>
        )}
      </section>

      <section className="mt-6 overflow-hidden rounded-2xl bg-white shadow-sm">
        <div className="border-b border-slate-100 px-6 py-4">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h2 className="text-lg font-bold text-slate-900">
                All Requesters
              </h2>
              <p className="mt-1 text-sm text-slate-500">
                Showing {requesters.length} of {requesterTotal} requesters
              </p>
            </div>

            <div className="flex flex-col gap-3 sm:flex-row">
              <input
                value={search}
                onChange={(event) => setSearch(event.target.value)}
                className="rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-slate-900"
                placeholder="Search name or email..."
              />

              <select
                value={status}
                onChange={(event) => setStatus(event.target.value)}
                className="rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-slate-900"
              >
                <option value="">All</option>
                <option value="active">Active</option>
                <option value="inactive">Inactive</option>
              </select>
            </div>
          </div>
        </div>

        {isLoading && (
          <div className="p-6 text-sm text-slate-500">
            Loading requesters...
          </div>
        )}

        {!isLoading && requesters.length === 0 && (
          <div className="p-6 text-sm text-slate-500">No requesters found.</div>
        )}

        {!isLoading && requesters.length > 0 && (
          <>
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-left text-sm">
                <thead className="bg-slate-50 text-slate-600">
                  <tr>
                    <th className="px-5 py-3 font-medium">Name</th>
                    <th className="px-5 py-3 font-medium">Email</th>
                    <th className="px-5 py-3 font-medium">Phone</th>
                    <th className="px-5 py-3 font-medium">Organization</th>
                    <th className="px-5 py-3 font-medium">Status</th>
                    <th className="px-5 py-3 font-medium">Tickets</th>
                    <th className="px-5 py-3 font-medium">Actions</th>
                  </tr>
                </thead>

                <tbody className="divide-y divide-slate-100">
                  {requesters.map((requester) => (
                    <tr key={requester.id} className="hover:bg-slate-50">
                      <td className="px-5 py-4 font-medium text-slate-900">
                        {requester.name}
                      </td>
                      <td className="px-5 py-4 text-slate-600">
                        {requester.email}
                      </td>
                      <td className="px-5 py-4 text-slate-600">
                        {requester.phone || "-"}
                      </td>
                      <td className="px-5 py-4 text-slate-600">
                        <div>{requester.company || "-"}</div>
                        {(requester.department || requester.jobTitle) && (
                          <div className="mt-1 text-xs text-slate-400">
                            {[requester.department, requester.jobTitle]
                              .filter(Boolean)
                              .join(" · ")}
                          </div>
                        )}
                      </td>
                      <td className="px-5 py-4">
                        <span
                          className={`rounded-full px-3 py-1 text-xs font-medium ${
                            requester.isActive
                              ? "bg-green-50 text-green-700"
                              : "bg-red-50 text-red-700"
                          }`}
                        >
                          {requester.isActive ? "Active" : "Inactive"}
                        </span>
                      </td>
                      <td className="px-5 py-4 text-slate-600">
                        {requester._count?.tickets ?? 0}
                      </td>
                      <td className="px-5 py-4">
                        <div className="flex gap-2">
                          <button
                            onClick={() => openEditModal(requester)}
                            className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50"
                          >
                            Edit
                          </button>
                          {requester.isActive && (
                            <button
                              onClick={() =>
                                handleDeactivateRequester(requester.id)
                              }
                              className="rounded-lg bg-red-600 px-3 py-2 text-sm font-medium text-white hover:bg-red-700"
                            >
                              Deactivate
                            </button>
                          )}
                        </div>
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>

            <div className="flex items-center justify-between border-t border-slate-100 px-6 py-4">
              <p className="text-sm text-slate-500">
                Page {pagination.page} of {pagination.totalPages || 1}
              </p>

              <div className="flex gap-2">
                <button
                  onClick={goToPreviousPage}
                  disabled={pagination.page <= 1}
                  className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  Previous
                </button>

                <button
                  onClick={goToNextPage}
                  disabled={pagination.page >= pagination.totalPages}
                  className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  Next
                </button>
              </div>
            </div>
          </>
        )}
      </section>

      {selectedRequester && (
        <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-slate-900/50 px-4 py-8">
          <div className="w-full max-w-4xl rounded-2xl bg-white p-6 shadow-xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-xl font-bold text-slate-900">
                  Edit Requester
                </h2>
                <p className="mt-1 text-sm text-slate-500">
                  Update requester profile and account status.
                </p>
              </div>

              <button
                onClick={closeEditModal}
                className="rounded-lg border border-slate-300 px-3 py-1 text-sm text-slate-600"
              >
                Close
              </button>
            </div>

            <div className="mt-6 space-y-6">
              <FormSection title="Basic Information">
                <TextField
                  label="Name"
                  value={editForm.name}
                  onChange={(value) => updateEditForm("name", value)}
                  required
                />
                <TextField
                  label="Email"
                  type="email"
                  value={editForm.email}
                  onChange={(value) => updateEditForm("email", value)}
                  required
                />
                <TextField
                  label="Password"
                  type="password"
                  value={editForm.password}
                  onChange={(value) => updateEditForm("password", value)}
                  helperText="Leave empty to keep the current password."
                />
              </FormSection>

              <FormSection title="Contact Information">
                <TextField
                  label="Phone"
                  value={editForm.phone}
                  onChange={(value) => updateEditForm("phone", value)}
                  required
                />
                <TextField
                  label="Country"
                  value={editForm.country}
                  onChange={(value) => updateEditForm("country", value)}
                  required
                />
                <TextField
                  label="City"
                  value={editForm.city}
                  onChange={(value) => updateEditForm("city", value)}
                  required
                />
                <TextField
                  label="Address"
                  value={editForm.address}
                  onChange={(value) => updateEditForm("address", value)}
                  required
                  className="md:col-span-2"
                />
              </FormSection>

              <FormSection title="Organization Details">
                <TextField
                  label="Company / Organization"
                  value={editForm.company}
                  onChange={(value) => updateEditForm("company", value)}
                  required
                />
                <TextField
                  label="Department"
                  value={editForm.department}
                  onChange={(value) => updateEditForm("department", value)}
                />
                <TextField
                  label="Job Title"
                  value={editForm.jobTitle}
                  onChange={(value) => updateEditForm("jobTitle", value)}
                />
              </FormSection>

              <FormSection title="Account Status">
                <StatusField
                  value={editForm.isActive}
                  onChange={(value) => updateEditForm("isActive", value)}
                />
              </FormSection>

              <div className="flex justify-end gap-3 pt-2">
                <button
                  onClick={closeEditModal}
                  className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700"
                >
                  Cancel
                </button>
                <button
                  onClick={handleUpdateRequester}
                  disabled={isUpdating}
                  className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white disabled:opacity-60"
                >
                  {isUpdating ? "Saving..." : "Save Changes"}
                </button>
              </div>
            </div>
          </div>
        </div>
      )}
    </div>
  );
}

function FormSection({
  title,
  children,
}: {
  title: string;
  children: ReactNode;
}) {
  return (
    <section>
      <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-500">
        {title}
      </h3>
      <div className="mt-3 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {children}
      </div>
    </section>
  );
}

function TextField({
  label,
  value,
  onChange,
  type = "text",
  required = false,
  helperText,
  className = "",
}: {
  label: string;
  value: string;
  onChange: (value: string) => void;
  type?: string;
  required?: boolean;
  helperText?: string;
  className?: string;
}) {
  return (
    <div className={className}>
      <label className="mb-1 block text-sm font-medium text-slate-700">
        {label}
        {required && <span className="text-red-500"> *</span>}
      </label>
      <input
        type={type}
        required={required}
        value={value}
        onChange={(event) => onChange(event.target.value)}
        className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-slate-900"
      />
      {helperText && (
        <p className="mt-1 text-xs leading-5 text-slate-500">{helperText}</p>
      )}
    </div>
  );
}

function StatusField({
  value,
  onChange,
}: {
  value: boolean;
  onChange: (value: boolean) => void;
}) {
  return (
    <div>
      <label className="mb-1 block text-sm font-medium text-slate-700">
        Status <span className="text-red-500">*</span>
      </label>
      <select
        value={value ? "active" : "inactive"}
        onChange={(event) => onChange(event.target.value === "active")}
        className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-slate-900"
      >
        <option value="active">Active</option>
        <option value="inactive">Inactive</option>
      </select>
    </div>
  );
}
